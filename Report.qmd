...
Racial profiling in law enforcement has been a debated issue, shaped by personal experiences and societal narratives. This divide is reflected in Kristina’s AP Statistics classroom, where she uses the NYPD stop-and-frisk dataset (2018–2023) to teach chi-square tests for independence. Her students analyze the association between race and the level of force used, leading to discussions that often mirror broader American views. Some students express skepticism about racial profiling, while others—particularly minority students— challenge their classmates skepticism by sharing personal accounts of being profiled.

These classroom experiences motivated us to explore whether the variables in the stop-and-frisk dataset could predict a suspect’s race or arrest status. This inquiry was further driven by the stark disparities revealed in the data. According to the [2020 US Census](https://www.census.gov/quickfacts/fact/table/newyorkcitynewyork/PST045223), Black individuals comprise 23% of New York City’s population but accounted for 58% of stop-and-frisk incidents from 2018–2023! Their goal was to determine whether the dataset could offer insights into systemic biases in law enforcement and inform broader societal discussions.

## Study Design and Sampling Methods

We analyzed a census of NYPD stop-and-frisk stops from 2018-2023. The dataset includes suspect demographic information, reasons for the stop, levels of force used, stop duration, arrest status, etc.

## Scientific Questions to Investigate

Initially, we sought to determine whether the information in the NYPD stop-and-frisk dataset could be used to predict a suspect's race. However, when our models accuracies failed to exceed that of random guessing, our focus shifted to the following:

 - **Can the 2018-2023 NYPD stop-and-frisk dataset be used to predict whether or not a suspect was arrested?** 
 - **Which variables carry the most importance when attempting to predict a suspect's arrest status?**
...
The anaylsis aimed to explore whether variables in the NYPD stop-and-frisk dataset (2018–2023) could predict a suspect’s race. When models failed to surpass random guessing, the focus shifted to predicting whether a suspect would be arrested. The analysis also sought to identify the most significant variables influencing arrest prediction outcomes.

### Results Interpretation

1.  **Predicting Race**: Models attempting to predict a suspect’s race using variables such as age, height, and weight did not exceed random guessing accuracy. This indicates the dataset lacked sufficient information to capture systemic profiling patterns.

2.  **Predicting Arrest Outcomes**: Refocusing the analysis on predicting arrest status proved more successful. Among all models fit, XGBoost demonstrated the best performance, achieving an accuracy of 72.9%. This model’s predictive power relied heavily on the following variables:

    i.  Weapon_Flag_Count
    ii. Stop_Duration_Minutes
    iii. Physical_Force_Handcuff_Suspect_FlagY
    iv. Suspect_Action_Counts

3.  **Key Predictive Variables**: Weapon_Flag_Count and Physical_Force_Handcuff_Supsect_FlagY are in the top 4 variables for each model. These findings align with expectations, as both factors directly relate to the severity of the incident and procedural escalation.

Shifting the focus to arrest led to predictions higher than random guessing. The models identified important predictors, with accuracy nearing 75%, demonstrating the dataset’s utility in analyzing procedural outcomes rather than profiling individuals.
...
The objective of this project was to determine whether the NYPD stop-and-frisk dataset (2018–2023) could be used to predict a suspect’s race and, if not, to explore the ability to predict arrest outcomes. When the initial models for race prediction failed to surpass random guessing, the analysis shifted to focus on arrest prediction, with the goal of identifying key variables driving procedural decisions.

The findings highlighted the dataset's limitations in capturing systemic patterns of racial profiling but demonstrated its usefulness in analyzing arrest outcomes. XGBoost emerged as the top-performing model, achieving an accuracy of 72.9% in predicting arrests. The most influential variables included Weapon_Flag_Count and Physical_Force_Handcuff_Suspect_FlagY, both of which directly relate to the severity of the stop and procedural escalation. These results underscore the importance of situational factors over demographic variables in driving arrest decisions.
...
#### Top Predictor Variables by Arrested Status
...
summarize_tuning <- function(models) {
  ...
  tuning_summary <- lapply(names(models), function(model_name) {
    ...
    if (!is.null(best_tune) && best_tune[[1]] != "none") {
      summary <- best_tune |>
        as.data.frame() |>
    ...
    # Return a data frame for each model with the updated name
    data.frame(
...
```{r plot-models, fig.height=7, fig.width=8}
lapply(names(models), function(model_name) {
  ...
  summary_table <- test_metrics_long |>
    ...
    map(~ {
      ...
      formatted <- .x |>
        ...
        pull(Model) |>
        paste(collapse = " \\newline ") # Combine all models for the metric
